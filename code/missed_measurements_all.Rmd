---
title: "Reduced number of RF measurements due to Covid-19"
author: 'Fred Ho'
output:
  html_document:
    df_print: paged
...
Estimate missed RF measurements in GDPPR using interrupted time series models to predict counterfactual number of measurements. ITS models include time trend prior to covid, Oxford stringency index, and change in time change since Covid.

Missed measurements is calcuated as observed number of measurement - counterfactual number of measurements as if stringency index = 0 and there's no trend change.
...
ggplot(tab_long %>% 
       ...
       aes(x=date, y=num, colour=scenario)) + 
  geom_line() + 
...
ggplot(tab_missed %>% 
  ...
  ylab('Reduced number of measurements (\'00,000)') + xlab('') + 
  facet_wrap(vars(mst), scales='free')
```
...
# Calculate total missed measurements
...
3.  Post-restriction (since Feb 2022)
...
tab_sum <- tab_missed %>% 
  mutate(
    period=case_when(
      date <= ymd('2021-06-01') ~ 'Lockdown', 
      date <= ymd('2022-03-01') ~ 'Post-lockdown', 
...
```{r table_missed_overall, echo=T}
tab_sum %>% 
  group_by(mst, period) %>% 
  summarise(
    sum=sum(sum), sum_lci=sum(sum_lci), sum_uci=sum(sum_uci), 
    mon=sum(mon), mon_lci=sum(mon_lci), mon_uci=sum(mon_uci), 
    .groups='drop'
  ) %>% 
  mutate(
    across(sum:mon_uci, function(.x) round(.x, 2)), 
  ) %>%
