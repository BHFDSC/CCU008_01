...
tab_combo %>% 
  ...
  summarise(
    delta.l = (sum(n_rf) - sum(pn.lci)) / sum(popu) *1000, 
    delta.u = (sum(n_rf) - sum(pn.uci)) / sum(popu) *1000,
  ...
  inner_join(
    tab_combo %>% 
      filter(date <= ymd(plot_end_date) & date >= ymd('2020-03-15')) %>% 
      summarise(
        delta.l = (sum(n_rf) - sum(pn.lci)) / sum(n_rf) *100, 
        delta.u = (sum(n_rf) - sum(pn.uci)) / sum(n_rf) *100,
  ...
  write_csv('output/tables/rate_deviations.csv')
